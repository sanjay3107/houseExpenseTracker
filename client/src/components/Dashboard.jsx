...
const Dashboard = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          const houseResponse = await axios.get(`${API_ENDPOINTS.HOUSE.DETAILS}?_t=${new Date().getTime()}`);
        ...
        try {
          ...
          const expensesByCategoryResponse = await axios.get(`${API_ENDPOINTS.EXPENSES.ALL}/summary/by-category`);
          expensesByCategoryData = expensesByCategoryResponse.data;
          
          // Fetch total expenses
          const totalExpensesResponse = await axios.get(`${API_ENDPOINTS.EXPENSES.ALL}/summary/total`);
  ...
  const categoryChartData = {
    labels: expensesByCategory.length > 0 ? expensesByCategory.map(item => item._id || item.category || 'Unknown') : ['No Data'],
    datasets: [
      {
        label: 'Expenses by Category',
        data: expensesByCategory.length > 0 ? expensesByCategory.map(item => item.total || item.amount || 0) : [0],
        backgroundColor: [
          'rgba(255, 99, 132, 0.6)',
          'rgba(54, 162, 235, 0.6)',
          'rgba(255, 206, 86, 0.6)',
  ...
  if (loading) {
    return (
      <div className="d-flex flex-column justify-content-center align-items-center" style={{ minHeight: '60vh' }}>
        <div className="spinner-grow text-primary mb-4" role="status" style={{ width: '3rem', height: '3rem' }}>
  ...
  return (
    <div className="dashboard">
      <div className="d-flex justify-content-between align-items-center border-bottom pb-3 mb-4">
        <h1 className="mb-0"><i className="bi bi-speedometer2 me-2 text-primary"></i>Dashboard</h1>
        <div>
        ...
        <Alert variant="info" className="mb-4 border-start border-4 border-primary shadow-sm">
          <div className="d-flex align-items-center">
            ...
            <div>
              ...
              <p className="mb-2 text-secondary">Start by adding details about your house to set up your personalized dashboard.</p>
              <div>
      ...
      <Row className="mb-4">
        <Col md={6} lg={3} className="mb-3">
          <Card className="h-100 shadow-sm border-0 bg-gradient" style={{ background: 'linear-gradient(135deg, #f8f9fa, #e9ecef)' }}>
            <Card.Body className="p-4">
              ...
              <h3 className="display-6 fw-bold text-primary mb-1">{formatIndianCurrency(totalExpenses || 0)}</h3>
        ...
        <Col md={6} lg={3} className="mb-3">
          <Card className="h-100 shadow-sm border-0 bg-gradient" style={{ background: 'linear-gradient(135deg, #f8f9fa, #e9f7ef)' }}>
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              {houseDetails && houseDetails.purchase_details?.purchase_price ? (
                <h3 className="display-6 fw-bold text-success mb-1">{formatIndianCurrency(houseDetails.purchase_details.purchase_price)}</h3>
              ) : (
            ...
            <Card.Footer className="bg-transparent border-top-0 pt-0">
              <Link to="/house-details" className="btn btn-sm btn-outline-success w-100">
                <i className="bi bi-house-gear me-1"></i> {houseDetails ? 'View house details' : 'Add house details'}
              </Link>
            </Card.Footer>
          </Card>
        </Col>
        ...
        <Col md={6} lg={3} className="mb-3">
          <Card className="h-100 shadow-sm border-0 bg-gradient" style={{ background: 'linear-gradient(135deg, #f8f9fa, #ffecef)' }}>
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              {houseDetails && houseDetails.loan_details?.loan_amount ? (
                <h3 className="display-6 fw-bold text-danger mb-1">{formatIndianCurrency(houseDetails.loan_details.loan_amount)}</h3>
              ) : (
                <h3 className="display-6 fw-bold text-muted mb-1">Not set</h3>
              )}
        ...
        <Col md={6} lg={3} className="mb-3">
          <Card className="h-100 shadow-sm border-0 bg-gradient" style={{ background: 'linear-gradient(135deg, #f8f9fa, #fff9e6)' }}>
            <Card.Body className="p-4">
              <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              {houseDetails && houseDetails.loan_details?.monthly_payment ? (
                <h3 className="display-6 fw-bold text-warning mb-1">{formatIndianCurrency(houseDetails.loan_details.monthly_payment)}</h3>
              ) : (
                <h3 className="display-6 fw-bold text-muted mb-1">Not set</h3>
              )}
      ...
      {expensesByCategory.length > 0 ? (
        <Row className="mb-4">
          <Col md={6} className="mb-3">
            <Card className="shadow-sm border-0">
              <Card.Header className="bg-white border-bottom d-flex justify-content-between align-items-center py-3">
                <h5 className="mb-0 fw-bold"><i className="bi bi-bar-chart-fill text-primary me-2"></i>Expenses by Category</h5>
          ...
          <Col md={6} className="mb-3">
            <Card className="shadow-sm border-0">
              <Card.Header className="bg-white border-bottom d-flex justify-content-between align-items-center py-3">
                <h5 className="mb-0 fw-bold"><i className="bi bi-pie-chart-fill text-primary me-2"></i>Expense Distribution</h5>
                <div className="dropdown">
                  <button className="btn btn-sm btn-light" type="button">
                    <i className="bi bi-three-dots-vertical"></i>
                  </button>
        ...
        <div className="mb-4 bg-light rounded-3 p-4 shadow-sm border-start border-4 border-info position-relative overflow-hidden">
          <div className="position-absolute" style={{ right: '-30px', top: '-20px', opacity: 0.1, transform: 'rotate(30deg)' }}>
          ...
          <div className="row align-items-center">
            ...
            <div className="col">
              ...
              <p className="mb-3 text-secondary">Add your first expense to visualize your spending with charts and analytics.</p>
      ...
      <Row>
        <Col>
          <Card className="shadow-sm border-0">
            <Card.Header className="bg-white border-bottom d-flex justify-content-between align-items-center py-3">
              <h5 className="mb-0 fw-bold"><i className="bi bi-receipt text-primary me-2"></i>Recent Expenses</h5>
              {expenses.length > 0 && (
              ...
              {expenses.length > 0 ? (
                      ...
                      {expenses.slice(0, 5).map(expense => (
                        <tr key={expense.id || expense._id}>
                          <td className="align-middle">
                            <span className="text-nowrap">{new Date(expense.date).toLocaleDateString()}</span>
                          </td>
                          <td className="align-middle">
                            <span className={`badge rounded-pill bg-${getCategoryColor(expense.category)} bg-opacity-75 py-2 px-3`}>
                              {expense.category}
                            </span>
                          </td>
                          <td className="align-middle">{expense.description}</td>
                          <td className="align-middle text-end fw-bold">
                            {typeof expense.amount === 'number' ? formatIndianCurrency(expense.amount) : 'N/A'}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="text-center py-5 bg-light rounded-3 my-3">
                  ...
                  <p className="text-muted mb-4 mx-auto" style={{ maxWidth: '500px' }}>
                    Start tracking your house expenses to get insights into your spending patterns and manage your budget efficiently.
                  </p>
