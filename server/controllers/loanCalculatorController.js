...
exports.calculateAmortizationSchedule = (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error calculating amortization schedule', error: error.message });
  }
};
...
exports.calculatePrepaymentImpact = (req, res) => {
  try {
    ...
    if (!loanAmount || !interestRate || !loanTerm || !prepaymentAmount || !prepaymentFrequency) {
      return res.status(400).json({
        message:
          'Missing required parameters: loanAmount, interestRate, loanTerm, prepaymentAmount, prepaymentFrequency',
      });
    }
    ...
    const prepaymentTotalPayment =
      prepaymentSchedule.reduce((sum, payment) => sum + payment.payment, 0) + totalPrepaymentAmount;
